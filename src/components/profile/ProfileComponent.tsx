...
interface ProfileComponentProps {
  userEmail?: string // Optional - if provided, shows another user's profile
  onClose?: () => void
}
...
export function ProfileComponent({ userEmail, onClose }: ProfileComponentProps) {
  ...
  // Add state to track how many responses to show
  ...
  useEffect(() => {
    ...
    const fetchUserData = async () => {
      ...
      try {
        ...
        // Load all responses for the user - without category parameter to get all responses
        const allResponses = await loadResponses()
        
        // Set flag to prevent refetching
        dataFetchedRef.current = true;
        
        // Filter out responses with undefined IDs
  ...
  }, [targetEmail, userData]) // Remove loadResponses from dependencies
  ...
  return (
    <div className="h-full p-6 bg-white rounded-lg shadow-sm border border-gray-100 flex flex-col mx-2 my-2 overflow-y-auto">
      <PageHeader title="Profil użytkownika" onClose={onClose} />
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
        <div className="bg-blue-50 p-4 rounded-lg border border-blue-100">
          ...
          <p className="text-sm text-blue-800 mt-1">
            {approvedResponses.length} zatwierdzonych odpowiedzi
          </p>
        </div>
        ...
        <div className="bg-amber-50 p-4 rounded-lg border border-amber-100">
          ...
          <p className="text-sm text-amber-800 mt-1">
            {responses.length} wszystkich odpowiedzi
          </p>
        </div>
